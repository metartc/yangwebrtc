...
#include <iostream>
#include <unistd.h>
#include <yangmeeting/include/YangMeetingDecoder.h>

#include "yangdecoder/YangDecoderFactory.h"

YangMeetingDecoder::YangMeetingDecoder(YangMeetingContext *pini) {
...
//YangRoomI* YangDecoderApp::getYangRoomI(){
	//return m_videoDec;
//}
...
void YangMeetingDecoder::initAudioDecoder(){
	...
	//	YangDecoderFactory df;
		m_audioDec=new YangAudioDecoderHandles(&m_ini->audio);
		m_audioDec->init();
...
void YangMeetingDecoder::initVideoDecoder(){
	...
	if(m_videoDec==NULL) {
		m_videoDec=new YangVideoDecoderHandles(&m_ini->video);
		m_videoDec->init();
