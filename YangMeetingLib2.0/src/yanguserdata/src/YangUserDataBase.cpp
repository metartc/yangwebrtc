...
#include <yanguserdata/include/YangCommandId.h>
...
void YangUserDataBase::clearData() {
	...
	if (m_ini) {
		m_ini->audiouids.clear();
		m_ini->videouids.clear();
		//m_ini->playvideouids.clear();
		//m_ini->playaudiouids.clear();
	}
}
void YangUserDataBase::loginHandle(int32_t pind, int32_t pisAdd) {
	...
	if (!pisAdd) {
		removeVectorUid(&m_ini->audiouids, yu->userId);
		removeVectorUid(&m_ini->videouids, yu->userId);
		//removeVectorUid(&m_ini->playaudiouids, yu->userId);
		//removeVectorUid(&m_ini->playvideouids, yu->userId);
	}
	...
	if (m_ini->receive)
		m_ini->receive->receiveMessage(strs);				//(perrCode);
	if (!pisAdd) {
...
int32_t YangUserDataBase::addUser(uint8_t *puserstr) {
	...
	char tuserstr[strsize];
	memset(tuserstr, 0, sizeof(tuserstr));
...
void YangUserDataBase::handleVideoStream(int32_t puserId, int32_t isAdd) {
	...
	int32_t ind = getIndex(yu->userId, &m_ini->videouids);
	if (isAdd) {
		if (ind == -1)
			m_ini->videouids.push_back(yu->userId);
		yu->camFlag = 2;
		//yang_post_message(YangM_Sys_Playvideo,yu->userId,NULL);
	} else {
		yu->camFlag = 1;
		if (ind > -1)
			m_ini->videouids.erase(m_ini->videouids.begin() + ind);
...
void YangUserDataBase::handleAudioStream(int32_t puserId, int32_t isAdd) {
	...
	if (index < 0)
		return;
	YangUser *yu = m_userList->at(index);
	int32_t ind = getIndex(yu->userId, &m_ini->audiouids);
	if (isAdd) {
		yu->micFlag = 2;
		if (ind == -1) {
			m_ini->audiouids.push_back(yu->userId);
			//yang_post_message(YangM_Sys_Playaudio, yu->userId, NULL);
		}
	} else {
		if (ind > -1) {
			m_ini->audiouids.erase(m_ini->audiouids.begin() + ind);
...
int32_t YangUserDataBase::logout(int32_t puid) {
	...
	removeVectorUid(&m_ini->audiouids, puid);
	removeVectorUid(&m_ini->videouids, puid);
	return Yang_Ok;
}
