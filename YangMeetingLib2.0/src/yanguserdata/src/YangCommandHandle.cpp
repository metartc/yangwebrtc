...
#include "../include/YangCommandHandle.h"
...
int32_t YangCommandHandle::sendCommand(int32_t pcommandId, uint8_t *p) {
	...
	char ss[128] = { 0 };
	sprintf(ss, "%d:%d:%d:%d:%s", m_ini->meeting.meetingId, m_ini->user.userId,
			m_ini->user.userId, pcommandId, p);
...
void YangCommandHandle::handleCommand(int32_t puserid, int32_t paimuserid,
	...
	case Yang_Command_AllowSpeak:
		if (paimuserid == m_ini->user.userId)
			yang_post_message(YangM_Sys_Pubaudio, m_ini->user.userId, NULL);
		break;
	case Yang_Command_DisableSpeak:
		if (paimuserid == m_ini->user.userId)
			yang_post_message(YangM_Sys_UnPubaudio, m_ini->user.userId, NULL);
		break;
