...
int32_t YangRtcPlayStream::initialize(YangRtcUserConfig *conf,
		YangRtcSourceDescription *stream_desc, YangReceiveCallback *cbk,
		YangRtpBuffer *rtpBuffer) {
	int32_t err = Yang_Ok;
	m_conf = conf;
	m_rtpBuffer = rtpBuffer;
	m_mixQueue.setAudioSize(conf->rtc_info->audioQueueCount);
	m_mixQueue.setVideoSize(conf->rtc_info->videoQueueCount);
...
YangRecvTrack::YangRecvTrack(int32_t uid, YangRtcUserConfig* conf,YangRtcSessionI *session,
	...
	m_mixQueue = pmixque;
	m_conf=conf;
	m_usingMixav=m_conf->rtc_info->mixAvqueue;
}
...
int32_t YangRecvTrack::on_nack(YangRtpPacket *pkt) {
	...
	if (!m_rtp_queue->update(seq, nack_first, nack_last)) {
		yang_info("NACK: too old seq %u, range [%u, %u]", seq,
				m_rtp_queue->m_begin, m_rtp_queue->m_end);
	}
	if (yang_rtp_seq_distance(nack_first, nack_last) > 0) {
		yang_trace("NACK: update seq=%u, nack range [%u, %u]", seq, nack_first,
				nack_last);
...
YangRtcAudioRecvTrack::YangRtcAudioRecvTrack(int32_t uid,YangRtcUserConfig* conf,
...
YangRtcVideoRecvTrack::YangRtcVideoRecvTrack(int32_t uid,YangRtcUserConfig* conf,
		YangRtcSessionI *session, YangRtcTrack *track_desc,
		YangRtpBuffer *rtpBuffer, YangMixQueue *pmixque) :
