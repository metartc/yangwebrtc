...
void YangRtcSession::setSendRequestCallback(YangSendRequestCallback *cb){
	m_sendReq=cb;
}
int32_t YangRtcSession::init(YangRtcUserConfig* pconf,YangSendUdpData *pudp, YangReceiveCallback *cbk,YangSendRequestCallback* reqcbk,
		YangStreamOptType role) {
	m_udp = pudp;
	m_conf=pconf;
	m_sendReq=reqcbk;
	if (m_dtls == NULL) {
	...
	if(m_rtpBuffer==NULL) m_rtpBuffer=new YangRtpBuffer(m_conf->streamConf->streamOptType);
	m_packet.init(m_rtpBuffer);
	if (role == Yang_Stream_Play) {
		if (m_play == NULL){
			m_play = new YangRtcPlayStream(this);
			m_play->initialize(m_conf,m_conf->source, cbk,m_rtpBuffer);
		...
		if (m_publish == NULL){
			...
			m_publish->init(m_conf->audioSsrc,m_conf->videoSsrc);
		}
	}
	return Yang_Ok;
}
...
void YangRtcSession::startTimers() {
	...
	if (m_conf->rtc_info&&m_conf->rtc_info->sendTwcc&&m_100ms&&!m_100ms->m_isStart)		m_100ms->start();
}
			...
			if (m_dtls->decodeHandshake(data, size) == Yang_Ok) {
				...
				if(m_dtls->getDtlsState()==YangDtlsStateClientDone) {
					printf("\ndtls shakehand success!.startTimers......audiossrc=%u............vssrc=%u,uid==%d",
							m_conf->audioSsrc,m_conf->videoSsrc,m_conf->streamConf->uid);
					startTimers();
					m_conf->state=1;

					if(m_sendReq) m_sendReq->sendRequest(m_conf->streamConf->uid,0,Yang_Req_Connected);
				}
			}
		}
...
int32_t YangRtcSession::send_video_meta(YangVideoStreamFrame* p)
{
...
int32_t YangRtcSession::publishVideo(YangVideoStreamFrame* p) {

	if(p->getFrametype()==9) return send_video_meta(p);
...
int32_t YangRtcSession::publishAudio(YangAudioStreamFrame *p) {
	YangRtpPacket *pkt=new YangRtpPacket();
	YangAutoFree(YangRtpPacket,pkt);
...
void YangRtcSession::do_request_keyframe(uint32_t ssrc){
	if(m_sendReq) m_sendReq->sendRequest(m_conf->streamConf->uid,ssrc,Yang_Req_Sendkeyframe);
}
