...
protected:
	...
	bool m_usingMixav;
public:
	YangRecvTrack(int32_t uid,YangRtcUserConfig* conf,YangRtcSessionI *session, YangRtcTrack *track_desc,YangRtpBuffer* rtpBuffer,YangMixQueue* pmixque,
			bool is_audio);
...
public:
	...
	void update_send_report_time(const YangNtp &ntp);
...
public:
	YangRtcAudioRecvTrack(int32_t uid,YangRtcUserConfig* conf,YangRtcSessionI *session,
			YangRtcTrack *track_desc,YangRtpBuffer* rtpBuffer,YangMixQueue* pmixque);
...
class YangRtcVideoRecvTrack: public YangRecvTrack,
		public IYangRtspPacketDecodeHandler {
public:
	YangRtcVideoRecvTrack(int32_t uid,YangRtcUserConfig* conf,YangRtcSessionI *session,
			YangRtcTrack *stream_descs,YangRtpBuffer* rtpBuffer,YangMixQueue* pmixque);
...
public:
	...
	int32_t initialize(YangRtcUserConfig* conf,YangRtcSourceDescription *stream_desc,YangReceiveCallback *cbk,YangRtpBuffer* rtpBuffer);
	int32_t send_rtcp_rr();
	int32_t send_rtcp_xr_rrtr();
	int32_t check_send_nacks();
