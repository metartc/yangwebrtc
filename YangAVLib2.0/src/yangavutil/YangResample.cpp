...
YangResample::YangResample() {
	...
	m_state=0;
	m_channel=2;
	m_in_sample=44100,m_out_sample=44100;
	unloadLib();
}
...
void YangResample::resample(const  short *pin,uint32_t  *pinLen, short* pout,uint32_t  *poutLen){
	if(m_channel==1){
		if(m_state) yang_speex_resampler_process_int(m_state, 0, pin, pinLen, pout, poutLen);
	}else{
		if(m_state) yang_speex_resampler_process_interleaved_int(m_state, pin, pinLen, pout, poutLen);
	}
}
