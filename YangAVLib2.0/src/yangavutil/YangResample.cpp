...
void YangResample::resample(const  short *pin,uint32_t  pinLen, short* pout,uint32_t  poutLen){
    ...
    m_out_len=poutLen;
	if(m_channel==1){
        if(m_state) yang_speex_resampler_process_int(m_state, 0, pin, &m_in_len, pout, &m_out_len);
	}else{
        if(m_state) yang_speex_resampler_process_interleaved_int(m_state, pin, &m_in_len, pout, &m_out_len);
	}
}
