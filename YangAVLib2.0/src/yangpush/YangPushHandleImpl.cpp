...
YangPushHandle* YangPushHandle::createPushHandle(YangContext* pini){
	return new YangPushHandleImpl(pini);
}
YangPushHandleImpl::YangPushHandleImpl(YangContext* pini) {
	m_cap=NULL;
	m_pub=NULL;
	m_ini=pini;
	m_cap=new YangPushPublish(m_ini);
...
void YangPushHandleImpl::publish(string url,string localIp,int32_t localPort) {
    ...
    if(m_pub==NULL){
    	m_pub=new YangRtcPublish(m_ini);
    }
