...
void YangPlayerBase::init(YangAudioInfo* audio){

	if(m_ydb==NULL) 		{
		m_ydb=new YangPlayerDecoder();
	...
	if(m_ypb==NULL) {
		...
		m_ypb->initAudioPlay(&m_ydb->m_audio);
...
void YangPlayerBase::startAudioPlay(YangAudioInfo* paudio) {
	//m_ydb->startDecoder();
	if(m_ypb){
