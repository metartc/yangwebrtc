...
YangAudioCaptureHandle::YangAudioCaptureHandle(YangAudioInfo *pini)
{
    ...
    hasPlayData=1;
    isFirst=1;
    m_aecBufferFrames=pini->aecBufferFrames;
...
//int32_t tcou=0;
    void YangAudioCaptureHandle::putBuffer(uint8_t *pBuffer,int32_t plen)
    {
		...
		if(m_aec) {
			...
			if(m_aecPlayBuffer&&m_aecPlayBuffer->m_size>m_aecBufferFrames){
				uint8_t* tmp=m_aecPlayBuffer->getAudioRef(&m_audioFrame);
				if(tmp)
				m_aec->echo_playback((short*)tmp);
				//printf("%d,",m_aecPlayBuffer->m_size);
				if(isFirst){
    ...
    void YangAudioCaptureHandle::putEchoBuffer( uint8_t *pBuffer,int32_t plen){
    	if(!isBuf) return;
    			if(m_aec) {
    			//	m_aec->echo_playback(pecho);
    				//	m_aec->echo_cancellation((short*)pBuffer,pecho,pcm);
    				m_aec->echo_capture((short*)pBuffer,pcm);
    				m_aec->preprocess_run(pcm);
