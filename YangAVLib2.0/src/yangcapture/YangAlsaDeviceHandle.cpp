...
#include <stdlib.h>
#include <malloc.h>
YangAlsaDeviceHandle::YangAlsaDeviceHandle(YangAudioInfo *pini) {
	m_para = pini;
	m_ahandle = new YangAudioCaptureHandle(pini);
	m_audioPlayCacheNum = m_para->audioPlayCacheNum;
	aIndex = 0;
	m_ret = 0;
	m_size = 0;
	m_loops = 0;
...
int32_t YangAlsaDeviceHandle::hasData() {
	if (!m_in_audioBuffer)
		return 0;
...
int32_t YangAlsaDeviceHandle::init() {
	...
	int32_t ret = alsa_device_open("default", m_sample, m_channel, m_frames);
			...
			if (m_in_audioBuffer && hasData()) {
				for (size_t i = 0; i < m_in_audioBuffer->size(); i++) {
					if (m_in_audioBuffer->at(i)
							&& m_in_audioBuffer->at(i)->m_size > 0) {
						...
						if (m_in_audioBuffer->at(i)->m_size
								> m_audioPlayCacheNum)
