...
YangWinAudioCapture::YangWinAudioCapture(YangAudioInfo *pini) { // @suppress("Class members should be properly initialized")
	m_para = pini;
	m_ahandle = new YangWinAudioCaptureHandle(pini);
...
void YangWinAudioCapture::setPreProcess(YangPreProcess *pp) {
	m_ahandle->m_player->m_preProcess = pp;
	if (pp) {
...
void YangWinAudioCapture::setInAudioBuffer(vector<YangAudioPlayBuffer*> *pal) {
	m_ahandle->m_player->setInAudioBuffer(pal);
}
