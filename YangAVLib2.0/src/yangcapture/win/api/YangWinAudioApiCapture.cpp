...
YangWinAudioApiCapture::YangWinAudioApiCapture(YangAudioInfo* pini)
{
    ...
    m_deviceIn=NULL;
     ...
     m_captureVolume=NULL;
      ...
      m_audioData.initOut(pini->sample, pini->channel);
}
...
int YangWinAudioApiCapture::initCapture() {
   ...
   // qDebug()<<"m_clientIn....."<<m_clientIn;
    ...
    m_blockSize=pWfxIn->nBlockAlign;
   ...
   m_audioData.initIn(pWfxIn->nSamplesPerSec, pWfxIn->nChannels);

    hr = m_clientIn->GetBufferSize(&m_bufferLength);
    if(FAILED(hr))  return yang_error_wrap(1," capture audioclient GetBufferSize fail...");
    hr = m_clientIn->SetEventHandle(m_samplesReadyEvent);
    if(FAILED(hr))  return yang_error_wrap(1," capture audioclient SetEventHandle fail...");
       ...
       HRESULT hr = m_captureClient->GetBuffer(
        ...
        if (SUCCEEDED(hr)) {
            ...
            if (pData) {
            	 ...
            	 audioFrame->nb=framesAvailable*m_blockSize;
