...
YangRecordCapture::YangRecordCapture(YangAudioInfo *paudio,
		YangVideoInfo *pvideo) {
	m_audio = paudio;
	m_video = pvideo;
	...
	m_pre_videoBuffer = new YangVideoBuffer(m_video->width, m_video->height, 12,
			m_video->bitDepth == 8 ? 1 : 2);
...
void YangRecordCapture::initAudio(YangPreProcess *pp) {
	if (m_out_audioBuffer == NULL) {
		if (m_audio->usingMono)
			m_out_audioBuffer = new YangAudioBuffer(m_audio->audioCacheNum);
		else
			m_out_audioBuffer = new YangAudioBuffer(m_audio->audioCacheNum);
	}
	if (m_audioCapture == NULL) {
		m_audioCapture = m_capture.createRecordAudioCapture(m_audio); //.createAudioCapture(m_ini);//new YangAudioCapture(m_ini);
...
void YangRecordCapture::initVideo() {
	if (m_out_videoBuffer == NULL)
		m_out_videoBuffer = new YangVideoBuffer(m_video->width, m_video->height,
				12, m_video->bitDepth == 8 ? 1 : 2);
	if (m_videoCapture == NULL) {
		m_videoCapture = m_capture.createVideoCapture(m_video); //new YangVideoCapture(m_ini);
...
void YangRecordCapture::startVr(char *pbg) {
	...
	if (m_vr_videoBuffer == NULL)
		m_vr_videoBuffer = new YangVideoBuffer(m_video->width, m_video->height,
				12, m_video->bitDepth == 8 ? 1 : 2);
	if (m_videoCapture) {
...
void YangRecordCapture::startScreen() {
	...
	if (m_screen_videoBuffer == NULL)
		m_screen_videoBuffer = new YangVideoBuffer(m_video->width,
				m_video->height, 12, m_video->bitDepth == 8 ? 1 : 2);
	if (m_videoCapture) {
...
void YangRecordCapture::startScreenLoop() {
	...
	while (m_isConvert == 1) {
		...
		if (m_screen_videoBuffer->m_size == 0) {
			yang_usleep(1000);
			continue;
		}
