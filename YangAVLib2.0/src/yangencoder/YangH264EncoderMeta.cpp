...
void YangH264EncoderMeta::yang_getSpsPps(YangH2645Conf *p264,
	...
	for (int32_t i = 0; i < iNal; ++i) {
		...
		case NAL_SPS:
			...
			/** printf("%d*****************************\n",p264->sps_len);
			 for(int32_t i=0;i<sps_len;i++){
			 printf("%2x,",p264->sps[i]);
			 }**/
			break;
		case NAL_PPS:
...
void YangH264EncoderMeta::yang_initVmd(YangVideoMeta *p_vmd,
	...
	if (!p_vmd->isInit) {
		...
		//   yang_getH264Config_Flv(&p_vmd->mp4Meta, p_vmd->flvMeta.buffer, &p_vmd->flvMeta.bufLen);
		p_vmd->isInit = 1;
	}
}
