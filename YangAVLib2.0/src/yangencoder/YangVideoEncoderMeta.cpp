...
void YangVideoEncoderMeta::yang_getConfig_Flv_H264(YangH2645Conf *p_264, uint8_t *configBuf,
		int32_t *p_configLen) {
	configBuf[0] = 0x17;
	configBuf[1] = 0x00;
	configBuf[2] = 0x00;
	...
	short slen = p_264->spsLen;		//spslen（short）
	slen = htons(slen);
	...
	*szTmp = 0x01;
	szTmp += 1;
	slen = p_264->ppsLen;		//spslen（short）
	slen = htons(slen);
...
void YangVideoEncoderMeta::yang_getConfig_Flv_H265(YangH2645Conf *p_264,
