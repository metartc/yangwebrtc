...
//	int32_t tmp = 0;
	//m_resample->init(2,m_ini->sample,16000);

	//m_input = new float[320];
	//if (!m_yap.usingMono) {
	m_frameSize=m_audioInfo.sample/50;
	int32_t floatFrame=m_frameSize*m_audioInfo.channel;
		m_in = new short[floatFrame];
		//temp = new char[1024 * 8];
	//}
	//m_in = new short[882*2];	//short in[m_frameSize];
	m_cbits = new uint8_t[MAX_PACKET_SIZE];
	m_input = new uint8_t[floatFrame*2];
	m_input1 = new short[floatFrame];
	m_isInit = 1;
	printf("\naudio encoder sample=%d,channel=%d",m_audioInfo.sample,m_audioInfo.channel);
}
...
int32_t YangAudioEncoderOpus::encoder(YangFrame* pframe,YangEncoderCallback *pcallback) {
	if (!m_encoder)		return 1;
		...
		//printf("ba%d,",bufLen);
		ret = yang_opus_encode(m_encoder, m_input1, m_frameSize*m_audioInfo.channel, m_cbits,	MAX_PACKET_SIZE);
		if (ret > 0 && pcallback){
