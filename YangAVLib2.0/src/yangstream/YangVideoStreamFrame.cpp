...
YangVideoStreamFrame::YangVideoStreamFrame() {
...
void YangVideoStreamFrame::init(int32_t transtype) {
	m_transType=transtype;
	if(m_transType!=2){
		if(m_videoBufs==NULL) m_videoBufs = new uint8_t[1024 * 128];
...
void YangVideoStreamFrame::initEncType(YangVideoEncDecType videoType,
		int32_t frametype) {
	if (frametype == 1) {
...
void YangVideoStreamFrame::initSpspps(uint8_t *buf){
...
void YangVideoStreamFrame::setVideoData(YangFrame* videoFrame,	YangVideoEncDecType videoType) {
	if(m_transType==2){
...
void YangVideoStreamFrame::setVideoMeta(uint8_t *p, int32_t plen, YangVideoEncDecType videoType) {
...
void YangVideoStreamFrame::setMetaTimestamp(int64_t timestamp){
	m_metaTime=timestamp;
}
void YangVideoStreamFrame::setFrametype(int32_t frametype){
	 m_frametype=frametype;
}

YangSpsppsConf* YangVideoStreamFrame::getSpsppsConf(){
	return m_spsppsConf;
}

int32_t YangVideoStreamFrame::getFrametype(){
	return m_frametype;
}
int32_t YangVideoStreamFrame::getVideoLen(){
	return m_videoLen;
}

int64_t YangVideoStreamFrame::getRtmpTimestamp() {
	return curVideotimestamp / 1000;
}
int64_t YangVideoStreamFrame::getTimestamp(){
	if(m_transType==0){
		return curVideotimestamp / 1000;
	}
		return curVideotimestamp * 9 / 100;
}
int64_t YangVideoStreamFrame::getMetaTimestamp(){
	return m_metaTime;
}
int64_t YangVideoStreamFrame::getTsTimestamp() {
	return curVideotimestamp * 9 / 100;
}
uint8_t* YangVideoStreamFrame::getVideoData() {

	return m_transType==2?m_src:m_videoBuffer;
}
