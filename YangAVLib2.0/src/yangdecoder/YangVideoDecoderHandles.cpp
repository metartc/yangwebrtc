...
YangVideoDecoderHandles::YangVideoDecoderHandles(YangVideoInfo *pini) {
	isInit = 0;
	m_isStart = 0;
	m_isConvert = 1;
...
void YangVideoDecoderHandles::addVideoStream(uint8_t *ps, int32_t puid,
	...
	if (pisAdd) {
		...
		m_decs->push_back(df.createVideoDecoder(aet, m_ini));
		...
		m_out_videoBuffer->push_back(
				new YangVideoBuffer(m_ini->bitDepth == 8 ? 1 : 2));
		m_out_videoBuffer->back()->m_uid = puid;
		m_out_videoBuffer->back()->init(wid, hei, 12);
...
void YangVideoDecoderHandles::startLoop() {
	m_isConvert = 1;
	uint8_t *srcVideo = new uint8_t[1024 * 512];
	uint8_t *temp;
	temp = srcVideo + 4;

	int32_t isFFmpeg = 1;		//(m_ini->videoDecoderType == 1 ? 1 : 0);
	int32_t index = 0;
	...
	while (m_isConvert == 1) {
		...
		if (t_decoder) {
			...
			if(t_decoder->decode(&videoFrame,yuvType,this)){
