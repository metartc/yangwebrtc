...
YangAudioDecoderHandles::YangAudioDecoderHandles(YangAudioInfo *pini) {
	m_isInit = 0;
	m_isStart = 0;
	m_isConvert = 1;
	...
	while (m_isConvert == 1) {
		...
		if (!t_decoder) {
			...
			if(iter != m_paramMap.end()){
				...
				param->encode=(YangAudioEncDecType)m_ini->audioDecoderType;
				param->sample=m_ini->sample;
				param->channel=m_ini->channel;
