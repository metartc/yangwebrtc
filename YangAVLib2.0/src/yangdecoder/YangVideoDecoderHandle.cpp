...
YangVideoDecoderHandle::YangVideoDecoderHandle(YangVideoInfo *pini) {
	isInit = 0;
	m_isStart = 0;
	m_isConvert = 1;
...
void YangVideoDecoderHandle::addVideoStream(uint8_t *ps, int32_t puid,
	...
	if (pisAdd) {
		...
		m_decs=df.createVideoDecoder(aet, m_ini);
		m_decs->m_uid = puid;
		m_decs->init();
		...
		if(m_out_videoBuffer){
			...
			m_out_videoBuffer->init(wid, hei, 12);
...
void YangVideoDecoderHandle::startLoop() {
	...
	while (m_isConvert == 1) {
		...
		if (m_decs) {
			...
			if(m_decs->decode(&videoFrame,yuvType,this)){
