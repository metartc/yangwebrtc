...
YangCertificate::YangCertificate() {
	ecdsa_mode = true;
	dtls_cert = NULL;
	dtls_pkey = NULL;
	eckey = NULL;
...
YangCertificate* YangCertificate::m_instance=NULL;
YangCertificate* YangCertificate::createCertificate(){
	if(m_instance==NULL) {
		m_instance=new YangCertificate();
		m_instance->initialize();
	}
	return m_instance;
}
int32_t YangCertificate::initialize() {
	int32_t err = Yang_Ok;
	...
	if (ecdsa_mode) { // By ECDSA, https://stackoverflow.com/a/6006898
		...
		yang_assert(EC_KEY_set_group(eckey, ecgroup) == 1);
		yang_assert(EC_KEY_generate_key(eckey) == 1);
		yang_assert(EVP_PKEY_set1_EC_KEY(dtls_pkey, eckey) == 1);
	...
	if (true) {
		...
		yang_assert(X509_set_pubkey(dtls_cert, dtls_pkey) == 1);
		yang_assert(X509_sign(dtls_cert, dtls_pkey, EVP_sha1()) != 0);
...
X509* YangCertificate::get_cert() {
	return dtls_cert;
}

EVP_PKEY* YangCertificate::get_public_key() {
	return dtls_pkey;
}

EC_KEY* YangCertificate::get_ecdsa_key() {
	return eckey;
}

std::string YangCertificate::get_fingerprint() {
	return fingerprint;
}

bool YangCertificate::is_ecdsa() {
	return ecdsa_mode;
}
