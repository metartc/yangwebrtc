...
void YangIni::initVideo(YangVideoInfo* video){
	...
	video->width = readIntValue("video", "width", 1280);
		...
		video->videoCaptureFormat = readIntValue("video", "videoCaptureFormat", 0);
		video->videoEncoderFormat = readIntValue("video", "videoEncoderFormat", 0);
       ...
       void YangIni::initSys(YangSysInfo *sys){
    	   ...
    	   sys->usingDataServer = readIntValue("sys", "usingDataServer", 0);
    		...
    		readStringValue("sys", "rtmpServerIP", sys->rtmpServerIP, "192.168.0.4");
       ...
       void YangIni::initEnc(YangVideoEncInfo *enc){
    	   ...
    	   enc->enc_threads = readIntValue("enc", "enc_threads", 2);
