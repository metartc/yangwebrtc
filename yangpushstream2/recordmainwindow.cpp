    ...
    , ui(new Ui::RecordMainWindow)
{
    ...
    //using gpu encode
    m_ini->video.videoEncHwType=YangV_Hw_Nvdia;//YangV_Hw_Intel,  YangV_Hw_Nvdia,
    m_ini->video.videoEncoderFormat=YangArgb;//YangI420;//YangArgb;
    m_ini->enc.createMeta=0;
#endif
#if Yang_HaveVr
    //using vr bg file name
    memset(m_ini->bgFilename,0,sizeof(m_ini->bgFilename));
    QSettings settingsread((char*)"yang_config.ini",QSettings::IniFormat);
    strcpy(m_ini->bgFilename, settingsread.value("sys/bgFilename",QVariant("d:\\bg.jpeg")).toString().toStdString().c_str());
#endif
