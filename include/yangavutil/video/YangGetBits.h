...
inline int yang_sign_extend(int val, unsigned bits) {
...
inline  unsigned int yang_get_bits(YangGetBitContext *s, int n);
inline  void yang_skip_bits(YangGetBitContext *s, int n);
inline  unsigned int yang_show_bits(YangGetBitContext *s, int n);
...
static  int yang_get_bits_count(YangGetBitContext *s) {
#if CACHED_BITSTREAM_READER
...
static  uint64_t yang_get_val(YangGetBitContext *s, unsigned n, int is_le)
{
	uint64_t ret;
	av_assert2(n>0 && n<=63);
	if (is_le) {
 ...
 *          is sufficient, too.
 */
static  void yang_skip_bits_long(YangGetBitContext *s, int n) {
#if CACHED_BITSTREAM_READER
	yang_skip_bits(s, n);
#else
#if UNCHECKED_BITSTREAM_READER
 ...
 */
static  int yang_get_xbits(YangGetBitContext *s, int n) {
#if CACHED_BITSTREAM_READER
...
#if !CACHED_BITSTREAM_READER
static  int yang_get_xbits_le(YangGetBitContext *s, int n) {
	register int sign;
	register int32_t cache;
	Yang_OPEN_READER(re, s);
	// av_assert2(n>0 && n<=25);
...
static  int yang_get_sbits(YangGetBitContext *s, int n) {
	register int tmp;
#if CACHED_BITSTREAM_READER
	av_assert2(n>0 && n<=25);
 ...
 * Read 0-25 bits.
 */
inline int yang_get_bitsz(YangGetBitContext *s, int n) {
	return n ? yang_get_bits(s, n) : 0;
}

inline  unsigned int yang_get_bits_le(YangGetBitContext *s, int n) {
#if CACHED_BITSTREAM_READER
	av_assert2(n>0 && n<=32);
	if (n > s->bits_left) {
		yang_refill_32(s);
...
inline  unsigned int yang_get_bits1(YangGetBitContext *s) {
#if CACHED_BITSTREAM_READER
	if (!s->bits_left)
	yang_refill_64(s);
...
inline  unsigned int yang_show_bits1(YangGetBitContext *s) {
 ...
 * Read 0-32 bits.
 */
inline  unsigned int yang_get_bits_long(YangGetBitContext *s, int n) {
	//av_assert2(n>=0 && n<=32);
	if (!n) {
		return 0;
#if CACHED_BITSTREAM_READER
 ...
 * Read 0-64 bits.
 */
inline  uint64_t yang_get_bits64(YangGetBitContext *s, int n) {
	if (n <= 32) {
 ...
 */
inline  int yang__get_sbits_long(YangGetBitContext *s, int n) {
 ...
 * Show 0-32 bits.
 */
inline  unsigned int yang_show_bits_long(YangGetBitContext *s, int n) {
...
inline  int yang_check_marker(void *logctx, YangGetBitContext *s,
		const char *msg) {
	int bit = yang_get_bits1(s);
	// if (!bit)
 ...
 */
inline  int yang_init_get_bits(YangGetBitContext *s,  uint8_t *buffer,
		int bit_size) {
	int buffer_size;
	int ret = 0;
 ...
 * @return 0 on success, AVERROR_INVALIDDATA if the buffer_size would overflow.
 */
inline  int yang_init_get_bits8(YangGetBitContext *s,  uint8_t *buffer,
		int byte_size) {
...
inline  int yang_set_idx(YangGetBitContext *s, int code, int *n, int *nb_bits,
Yang_TYPE (*table)[2]) {
	unsigned idx;
 ...
 */
inline int Yang_GET_VLC2(YangGetBitContext *s, Yang_TYPE (*table)[2], int bits,
		int max_depth) {
#if CACHED_BITSTREAM_READER
	int nb_bits;
...
inline  int yang_decode012(YangGetBitContext *gb) {
	int n;
	n = yang_get_bits1(gb);
	if (n == 0)
		return 0;
...
inline  int yang_decode210(YangGetBitContext *gb) {
...
inline  int yang_get_bits_left(YangGetBitContext *gb) {
	return gb->size_in_bits - yang_get_bits_count(gb);
}

inline  int yang_skip_1stop_8data_bits(YangGetBitContext *gb) {
