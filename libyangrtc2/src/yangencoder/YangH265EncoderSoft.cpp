...
int32_t YangH265EncoderSoft::encode(YangFrame* pframe,  YangEncoderCallback* pcallback){
	...
	for (uint32_t i = 0; i < m_i265Nal; i++) {
		...
		if (m_265Nal[i].type >= YANG_NAL_UNIT_CODED_SLICE_BLA&&m_265Nal[i].type <= YANG_NAL_UNIT_RESERVED_23)
	...
	//yang_trace("\n%d_%d:", destLength, (m_vbuffer[0] & 0x7e) >> 1);
	//for (int i = 0; i < 50; i++)		yang_trace("%02x,", (uint8_t )pframe->payload[i]);
	if (pcallback)
