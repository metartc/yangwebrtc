...
YangRtcSession::YangRtcSession() {
    ...
    m_10ms = new YangTimer();
    m_10ms->setTaskId(10);
    m_10ms->setTimelen(16);
    m_10ms->setTask(this);
...
void YangRtcSession::setSsrc(uint32_t audioSsrc,uint32_t videoSsrc){
	...
	m_packetWrap.m_audioSsrc=audioSsrc;
	m_packetWrap.m_videoSsrc=videoSsrc;
...
void YangRtcSession::startTimers() {
    if (m_10ms&&!m_10ms->m_isStart)		m_10ms->start();
...
{
     return  m_packetWrap.package_stap_a(p);
}
...
int32_t YangRtcSession::publishVideo(YangStreamCapture* p) {
	...
	return m_packetWrap.on_h264_video(p);

}

int32_t YangRtcSession::publishAudio(YangStreamCapture *p) {
	return m_packetWrap.on_audio(p);
