...
YangRtcVideoRecvTrack::YangRtcVideoRecvTrack(int32_t uid, YangRtcContext *conf,
	...
	m_hasReceiveStap=-1;
	m_cache_video_pkts=new YangRtcPacketCache[s_cache_size];
	memset(m_cache_video_pkts, 0, s_cache_size*sizeof(YangRtcPacketCache));
	m_video_buffer=new char[1024*256];
	memset(m_video_buffer,0,1024*256);
}


YangRtcVideoRecvTrack::~YangRtcVideoRecvTrack() {
	yang_deleteA(m_cache_video_pkts);
			...
			if (pkt->nb > 0) {
				if (pkt->fua2.start) {
					...
					*p++=pkt->fua2.nri | pkt->fua2.nalu_type;
					...
					if (pkt->fua2.end) {
						//write nalu_len back
						yang_put_be32(m_video_buffer+5,nalu_len);
					}
				}
...
int32_t YangRtcVideoRecvTrack::packet_video_key_frame(YangRtpPacket *pkt) {
	...
	if (pkt->m_nalu_type==kStapA&&is_keyframe(pkt)) {
		...
		if (NULL == sps || NULL == pps) {
			...
			m_hasReceiveStap=pkt->m_header.get_timestamp();
			m_stap.reset();
		}
	}
	if (-1 == m_key_frame_ts) {
...
void YangRtcVideoRecvTrack::clear_cached_video() {
	for (size_t i = 0; i < s_cache_size; i++) {
		if (m_cache_video_pkts[i].in_use) {
			...
			memset(&m_cache_video_pkts[i],0,sizeof(struct YangRtcPacketCache));
			/**m_cache_video_pkts[i].payload=NULL;
			m_cache_video_pkts[i].sn = 0;
			m_cache_video_pkts[i].ts = 0;
			m_cache_video_pkts[i].nalu_type = 0;
			m_cache_video_pkts[i].nb = 0;
			m_cache_video_pkts[i].end = false;
			m_cache_video_pkts[i].in_use = false;**/
		}
	}
}
...
bool YangRtcVideoRecvTrack::check_frame_complete(const uint16_t start,
	...
	for (uint16_t i = 0; i < cnt; ++i) {
		...
		YangRtcPacketCache *pkt = &m_cache_video_pkts[index];
		//YangRtpFUAPayload2 *fua_payload =dynamic_cast<YangRtpFUAPayload2*>(pkt->payload());
		if (pkt->nalu_type == kFuA) {
			if (pkt->fua2.start) {
				++fu_s_c;
			}
...
bool YangRtcVideoRecvTrack::is_keyframe(YangRtpPacket* pkt){
	// It's normal H264 video rtp packet
		if (pkt->m_nalu_type == kStapA) {
			...
			YangAvcNaluType nalu_type = (YangAvcNaluType) (pkt->m_payload[3] & kNalTypeMask);
								if (nalu_type == YangAvcNaluTypeSPS||nalu_type == YangAvcNaluTypePPS) {
									return true;
								}
		} else if (pkt->m_nalu_type == kFuA) {

			uint8_t v = pkt->m_payload[0];
			if(YangAvcNaluTypeIDR== YangAvcNaluType(v & kNalTypeMask)) return true;

		} else {
			if ((pkt->m_nalu_type==YangAvcNaluTypeIDR) || ( pkt->m_nalu_type==YangAvcNaluTypeSPS)
...
bool YangRtcVideoRecvTrack::is_keyframe(YangRtcPacketCache* pkt){
	// It's normal H264 video rtp packet
		if (pkt->nalu_type == kStapA) {
			...
			uint8_t v = pkt->payload[0];
			if(YangAvcNaluTypeIDR== YangAvcNaluType(v & kNalTypeMask)) return true;

		} else {
			if ((pkt->nalu_type==YangAvcNaluTypeIDR) || ( pkt->nalu_type==YangAvcNaluTypeSPS)
...
int32_t YangRtcVideoRecvTrack::on_rtp(YangRtpPacket *src) {
	...
	//src->m_header.
	uint8_t v = (uint8_t) (src->m_payload[0] & kNalTypeMask);
	src->m_nalu_type = YangAvcNaluType(v);
...
void YangRtcVideoRecvTrack::copy(YangRtpPacket *src, YangRtcPacketCache *pkt) {
  ...
  if (pkt->nalu_type == kFuA) {
		YangBuffer buf(src->m_payload, src->m_nb);
		yang_parseFua2(&buf,&pkt->fua2);
