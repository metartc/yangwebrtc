...
int32_t YangRtpPacketWrap::on_audio(YangStreamCapture *audioFrame) {
    ...
    m_audioRawPacket.m_header.set_payload_type(kAudioPayloadType);
    m_audioRawPacket.m_header.set_ssrc(m_audioSsrc);
    m_audioRawPacket.m_frame_type = YangFrameTypeAudio;
    m_audioRawPacket.m_header.set_marker(true);

    m_audioRawPacket.m_header.set_sequence(m_audioSeq++);
    m_audioRawPacket.m_header.set_timestamp(audioFrame->getAudioTimestamp());
    m_audioRawPacket.m_header.set_padding(0);
    m_audioRawPacket.m_payload_type = YangRtspPacketPayloadTypeRaw;

    m_raw_audio.m_payload = m_videoBuf;
    m_raw_audio.m_size = audioFrame->getAudioLen();
    memcpy(m_raw_audio.m_payload, audioFrame->getAudioData(), m_raw_audio.m_size);
...
int32_t YangRtpPacketWrap::package_single_nalu(YangStreamCapture *videoFrame) {
    ...
    m_videoRawPacket.m_header.set_payload_type(kVideoPayloadType);
    m_videoRawPacket.m_header.set_ssrc(m_videoSsrc);
    m_videoRawPacket.m_frame_type = YangFrameTypeVideo;
    m_videoRawPacket.m_header.set_sequence(m_videoSeq++);
    m_videoRawPacket.m_header.set_timestamp(videoFrame->getVideoTimestamp());
    m_videoRawPacket.m_header.set_marker(true);
    m_videoRawPacket.m_payload_type = YangRtspPacketPayloadTypeRaw;
    m_raw_video.m_payload = m_videoBuf;
    m_raw_video.m_size = videoFrame->getVideoLen();
    memcpy(m_raw_video.m_payload, videoFrame->getVideoData(),  m_raw_video.m_size);
...
int32_t YangRtpPacketWrap::package_single_nalu(char *p, int32_t plen,
    ...
    m_videoRawPacket.m_frame_type = YangFrameTypeVideo;
    m_videoRawPacket.m_header.set_sequence(m_videoSeq++);
    m_videoRawPacket.m_header.set_timestamp(timestamp);


    m_videoRawPacket.m_payload_type = YangRtspPacketPayloadTypeRaw;
    m_raw_video.m_payload = m_videoBuf;
    m_raw_video.m_size = plen;
    memcpy(m_raw_video.m_payload, p, plen);
    if ((err = encodeVideo(&m_videoRawPacket)) != Yang_Ok) {
        return yang_error_wrap(err, "encode packet");
    }
...
int32_t YangRtpPacketWrap::package_fu_a(YangStreamCapture *videoFrame,
    ...
    for (int32_t i = 0; i < num_of_packet; ++i) {
        ...
        m_videoFuaPacket.m_header.set_payload_type(kVideoPayloadType);
        m_videoFuaPacket.m_header.set_ssrc(m_videoSsrc);
        m_videoFuaPacket.m_frame_type = YangFrameTypeVideo;
        m_videoFuaPacket.m_header.set_sequence(m_videoSeq++);
        m_videoFuaPacket.m_header.set_timestamp(videoFrame->getVideoTimestamp());
        m_videoFuaPacket.m_header.set_marker(bool(i == num_of_packet - 1));
...
int32_t YangRtpPacketWrap::encodeVideo(YangRtpPacket *pkt) {
    int err = 0;
...
int32_t YangRtpPacketWrap::encodeAudio(YangRtpPacket *pkt) {
    int err = 0;
    m_buf.init(m_rtpBuffer->getBuffer(), kRtpPacketSize);
...
int32_t YangRtpPacketWrap::package_stap_a(YangStreamCapture *videoFrame) {
    ...
    m_videoStapPacket.m_header.set_payload_type(kVideoPayloadType);
    m_videoStapPacket.m_header.set_ssrc(m_videoSsrc);
    m_videoStapPacket.m_frame_type = YangFrameTypeVideo;
    m_videoStapPacket.m_nalu_type = (YangAvcNaluType) kStapA;
    m_videoStapPacket.m_header.set_marker(false);
    m_videoStapPacket.m_header.set_sequence(m_videoSeq++);
    m_videoStapPacket.m_header.set_timestamp(0);
