...
int32_t YangRtcPlayStream::on_rtp(char *data, int32_t nb_data) {
	...
	if ((err = m_session->get_srtp()->dec_rtp(plaintext, &nb_plaintext))
			!= Yang_Ok) {
		    return yang_error_wrap(err,"decrtp error nblen=%d",nb_plaintext);
	}
...
int32_t YangRtcPlayStream::on_rtp_plaintext(char *plaintext,
	...
	m_rtp.reset();
	YangBuffer buf(m_rtp.wrap(m_rtpBuffer, plaintext, nb_plaintext),
			nb_plaintext);
	// @remark Note that the pkt might be set to NULL.
	err = do_on_rtp_plaintext(&m_rtp, &buf);
