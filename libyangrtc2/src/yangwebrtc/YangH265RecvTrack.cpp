...
int32_t YangH265RecvTrack::packet_video_key_frame(YangRtpPacket *pkt) {
	...
	if (pkt->m_nalu_type==kStapA) {
		YangBuffer stap_buf(pkt->m_payload, pkt->m_nb);
	 	 yang_reset_h2645_stap(&m_stapData);
	     yang_decode_h265_stap(&stap_buf,&m_stapData);
		YangSample *sps = yang_get_stap_sps(&m_stapData);//m_stap.get_sps();
		YangSample *pps = yang_get_stap_pps(&m_stapData);//m_stap.get_pps();
		if (NULL == sps || NULL == pps) {
			...
			//TODO: call api
			*p++ = 0x1c; // type(4 bits): key frame; code(4bits): hevc
			...
			*p++ = 0xff;
			*p++ = 0xe1;
			yang_put_be16(p, sps->nb);
			p += 2;
			memcpy(p, sps->bytes, sps->nb);
			p += sps->nb;
			*p++ = 0x01;
			yang_put_be16(p, pps->nb);
			p += 2;
			memcpy(p, pps->bytes, pps->nb);
			p += pps->nb;

			m_hasReceiveStap = pkt->m_header.sequence;
			yang_reset_h2645_stap(&m_stapData);
			if ((err = put_frame_video(m_video_buffer, 0, p - m_video_buffer))
					!= Yang_Ok) {
				return err;
			}
...
int32_t YangH265RecvTrack::put_frame_mixvideo(char *p, int64_t timestamp,
		int32_t nb) {
...
int32_t YangH265RecvTrack::check_send_nacks() {
	int32_t err = Yang_Ok;
...
int32_t YangH265RecvTrack::notify(int puid,YangRtcMessageType mess){
