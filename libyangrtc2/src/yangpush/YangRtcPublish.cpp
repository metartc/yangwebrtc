			...
			if (videoFrame.frametype == YANG_Frametype_I) {
				...
				if (m_vmd) {
					...
					if (!vmd->isInit) {
						...
						yang_createMeta(vmd, &videoFrame);
						yang_getConfig_Flv_H264(&vmd->mp4Meta,vmd->livingMeta.buffer,&vmd->livingMeta.bufLen);
					}
					data.setVideoMeta(vmd->livingMeta.buffer,vmd->livingMeta.bufLen, videoType);
					ret = stream->publishVideoData(&data);

					memset(&nalu, 0, sizeof(YangNaluData));
                    yang_parseNalu(&videoFrame, &nalu);
