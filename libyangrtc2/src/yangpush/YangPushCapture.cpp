...
void YangPushCapture::initScreen(){
	//if(m_screen_pre_videoBuffer==NULL) m_screen_pre_videoBuffer = new YangVideoBuffer(m_context->video.bitDepth==8?1:2);
	//if(m_screen_out_videoBuffer==NULL) m_screen_out_videoBuffer = new YangVideoBuffer(m_context->video.bitDepth==8?1:2);
...
void YangPushCapture::addVr(){
	if (m_screen_pre_videoBuffer == NULL)
		m_screen_pre_videoBuffer = new YangVideoBuffer(m_context->video.width,
				m_context->video.height, 12, m_context->video.bitDepth == 8 ? 1 : 2);

	m_screen_pre_videoBuffer->resetIndex();

	m_pre_videoBuffer->resetIndex();
	m_out_videoBuffer->resetIndex();
	m_videoCapture->setPreVideoBuffer(m_screen_pre_videoBuffer);
