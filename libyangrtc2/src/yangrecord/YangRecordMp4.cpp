...
void YangRecordMp4::startLoop() {
	...
	while (m_isConvert == 1) {
		if (m_in_videoBuffer->m_size > 5) {
			yang_info("write cache big..%d\n", m_in_videoBuffer->m_size);
		}
		if (m_in_videoBuffer->m_size ==0 &&  m_in_audioBuffer->m_size ==0) {
			...
			if(m_in_audioBuffer->m_size>1) writeAudioData();
			if(m_in_videoBuffer->m_size){
				writeVideoData();

			}

	}
	//while (m_in_videoBuffer->m_size> 1 || m_in_audioBuffer->m_size > 0) {
	//	writeAudioData();
		//writeVideoData();
	//}
