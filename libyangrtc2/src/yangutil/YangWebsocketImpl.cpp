...
int yang_websocket_callback(struct lws *wsi, enum lws_callback_reasons reason,
    ...
    **/
	...
	case LWS_CALLBACK_RECEIVE_CLIENT_HTTP_READ:
		yang_info("LWS_CALLBACK_RECEIVE_CLIENT_HTTP_READ");
		break;
	case LWS_CALLBACK_RECEIVE_CLIENT_HTTP:
	{
		yang_trace("\nLWS_CALLBACK_RECEIVE_CLIENT_HTTP...");
		//if(in) yang_trace("\nLWS_CALLBACK_RECEIVE_CLIENT_HTTP:%s",(uint8_t*)in);
		char buffer[1024 + LWS_PRE];
		char *px = buffer + LWS_PRE;
		int lenx = sizeof(buffer) - LWS_PRE;
		if (obj->yang_lws_http_client_read(wsi, &px, &lenx) < 0)	return -1;
		 yang_trace("\nLWS_CALLBACK_RECEIVE_CLIENT_HTTP_READ:%s",(uint8_t*)buffer);
		 break;
	}
	case LWS_CALLBACK_CLIENT_APPEND_HANDSHAKE_HEADER:
	{

		yang_info("LWS_CALLBACK_CLIENT_APPEND_HANDSHAKE_HEADER");
		if(1) break;
		unsigned char **p = (unsigned char **)in, *end = (*p) + len;
		if (obj->yang_lws_add_http_header_by_token(wsi, WSI_TOKEN_HTTP_COOKIE,(const unsigned char*)"a=b", 3, p, end)){
			yang_error("lws_add_http_header_by_token error");
			return -1;
		}
		...
		if (obj->yang_lws_add_http_header_by_token(wsi, WSI_TOKEN_HTTP_CONTENT_LENGTH,
				(unsigned char*) "29", 2, p, end))
			return -1;
		if (obj->yang_lws_add_http_header_by_token(wsi, WSI_TOKEN_HTTP_CONTENT_TYPE,
				(unsigned char*) "application/x-www-form-urlencoded", 33, p,
				end))
			return -1;


		obj->yang_lws_client_http_body_pending(wsi, 1);
		obj->yang_lws_callback_on_writable(wsi);**/
		//yang_trace("\nLWS_CALLBACK_RECEIVE_CLIENT_HTTP_READ:%s",(uint8_t*)in);
		break;
	}
	...
	{
		yang_trace("\n*****LWS_CALLBACK_CLIENT_HTTP_WRITEABLE size==%d......................\n",obj->m_messages.size());
		if (obj->m_messages.size() >= 0)	break;
		yang_trace("\n********************send data start.......................\n");
		uint8_t buf[2048];
		...
		//obj->yang_lws_write(wsi, buf + LWS_PRE, data->nb, LWS_WRITE_TEXT);
		//memcpy(buf,data->payload,data->nb);
		obj->yang_lws_write(wsi, buf + LWS_PRE, data->nb, LWS_WRITE_HTTP);
		obj->yang_lws_client_http_body_pending(wsi, 0);
...
int YangWebsocketImpl::connectServer(std::string url) {
	...
	if(yang_janus_url_parse((char*)url.c_str(),&m_urlData)) return 1;
