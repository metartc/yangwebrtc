...
int32_t YangCertificate::initialize() {
	...
	if (ecdsa_mode) { // By ECDSA, https://stackoverflow.com/a/6006898
        ...
        if(EC_KEY_set_group(eckey, ecgroup) != 1) yang_error("EC_KEY_set_group error");
        if(EC_KEY_generate_key(eckey) != 1) yang_error("EC_KEY_generate_key error");
        if(EVP_PKEY_set1_EC_KEY(dtls_pkey, eckey) != 1) yang_error("EVP_PKEY_set1_EC_KEY error");
