...
void YangCLog::log(int32_t level, const char *fmt, ...) {
	...
	char cur_time[32] = { 0 };
    int64_t cur_time_msec = yang_get_milli_time();
	...
	if (m_hasLogFile) {
		...
		char sf[4196];
		memset(sf, 0, 4196);
        int32_t sfLen = sprintf(sf, "%s:%" PRId64" Yang %s: %s\n", cur_time,
...
int32_t yang_error_wrap(int32_t errcode, const char *fmt, ...) {
    ...
    printf("%s:%" PRId64" Yang Error(%d): %s\n", cur_time, cur_time_msec, errcode,
			buf);
	...
	if (YangCLog::m_hasLogFile) {
		...
		char sf[4196];
		memset(sf, 0, 4196);
        int32_t sfLen = sprintf(sf, "%s:%" PRId64" Yang Error(%d): %s\n", cur_time,
